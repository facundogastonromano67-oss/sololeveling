...
function renderGeneral(){
 ...
 const safeAvatar=typeof normalizeAvatarData==='function'?normalizeAvatarData(state.avatar):'';
 const avatar=`<span>${esc((state.player?.name||'G').slice(0,1).toUpperCase())}</span>`;
 root.innerHTML=`
 ...
 if(safeAvatar){const holder=root.querySelector('.v20-avatar'),img=document.createElement('img');img.alt='Avatar';img.src=safeAvatar;holder?.replaceChildren(img)}
...
function weekDateFor(day){const n=new Date(),monday=new Date(n),offset=(n.getDay()+6)%7,target=(Number(day)+6)%7;monday.setDate(n.getDate()-offset+target);return dateKeySafe(monday)}
function prevLog(ex,currentDate){return Object.entries(state.trainingDetailedLogs||{}).filter(([d])=>d<currentDate).sort(([a],[b])=>b.localeCompare(a)).map(([,v])=>{const exercises=v.exercises||{};return exercises[ex.id]||Object.values(exercises).find(x=>(ex.catalogId&&x.catalogId===ex.catalogId)||(!x.catalogId&&x.name===ex.name))}).find(Boolean)||null}
...
function persistTrainingInputs(day,date){
 ...
 day.exercises.forEach(ex=>{const old=log.exercises?.[ex.id]||{};const inputs=QA(`[data-v20-ex="${CSS.escape(ex.id)}"]`),sets=[];inputs.forEach(inp=>{const i=+inp.dataset.set;sets[i]=sets[i]||{done:!!old.sets?.[i]?.done};sets[i][inp.dataset.field]=inp.value});if(sets.length)log.exercises[ex.id]={name:ex.name,catalogId:ex.catalogId||'',sets:sets.filter(Boolean)}});
...
function exerciseCard(ex,log,date,weekday,idx,activeIdx){
 ...
 const prev=prevLog(ex,date);const prevValid=prev?.sets?.map(s=>({w:Number(s.weight)||0,r:Number(s.reps)||0})).filter(x=>x.w||x.r)||[];const last=prevValid.length?[...prevValid].sort((a,b)=>(b.w*b.r)-(a.w*a.r))[0]:null;
...
function renderTraining(){
 ...
 root.onclick=e=>{
  ...
  const done=e.target.closest('[data-v20-set-done]');if(done){persistTrainingInputs(day,date);const [id,si]=done.dataset.v20SetDone.split(':');const ex=day?.exercises.find(x=>x.id===id);if(!ex)return;const x=log.exercises[id]||(log.exercises[id]={name:ex.name,catalogId:ex.catalogId||'',sets:defaultSets(ex)});x.sets[+si]=x.sets[+si]||{};x.sets[+si].done=!x.sets[+si].done;save();if(x.sets.every(s=>s.done)){const ni=Math.min(day.exercises.length-1,day.exercises.findIndex(z=>z.id===id)+1);sessionStorage.setItem('v20ActiveExercise',String(ni))}renderTraining();return}
  const add=e.target.closest('[data-v20-add-set]');if(add){persistTrainingInputs(day,date);const ex=day?.exercises.find(x=>x.id===add.dataset.v20AddSet);if(!ex)return;const x=log.exercises[ex.id]||(log.exercises[ex.id]={name:ex.name,catalogId:ex.catalogId||'',sets:defaultSets(ex)});x.sets.push({reps:'',weight:'',rpe:'',done:false});save();renderTraining();return}
