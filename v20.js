...
function exerciseCard(ex,log,date,weekday,idx,activeIdx){
 ...
 const completed=saved.sets.filter(s=>s.done).length,isDone=saved.sets.length&&completed===saved.sets.length,isActive=idx===activeIdx;
 if(!isActive)return `<article class="v20-exercise v20-exercise-collapsed ${isDone?'done':''}" data-v20-open-ex="${idx}"><div class="v20-ex-mini"><div><span>EJERCICIO ${idx+1}${isDone?' · COMPLETADO':''}</span><h3>${esc(ex.name)}</h3><p>${esc(ex.prescription)}${ex.muscle?` · ${esc(ex.muscle)}`:''}</p></div><div class="v20-mini-status"><b>${completed}/${saved.sets.length}</b><small>series</small><button data-v20-open-ex="${idx}">Abrir</button></div></div></article>`;
 ...
 <div class="v20-ex-actions"><button class="v20-add-set" data-v20-add-set="${esc(ex.id)}">+ Agregar serie</button><button class="v20-rest-btn" data-v20-rest="150">Descanso · 2:30</button></div></article>`;
}
