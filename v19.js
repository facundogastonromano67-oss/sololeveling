...
function weekDateFor(day){const now=new Date(),delta=day-now.getDay();const d=new Date(now);d.setDate(now.getDate()+delta);return (typeof dateKey==='function'?dateKey(d):d.toISOString().slice(0,10));}
function previousTrainingLog(exId,currentDate){
 const logs=state.trainingDetailedLogs||{};return Object.entries(logs).filter(([d])=>d<currentDate).sort(([a],[b])=>b.localeCompare(a)).map(([,v])=>v.exercises?.[exId]).find(Boolean)||null;
}
...
function exerciseLogger(ex,log,date,idx){
 const saved=log.exercises[ex.id]||{sets:[{reps:'',weight:'',rpe:''},{reps:'',weight:'',rpe:''},{reps:'',weight:'',rpe:''}]};
 const prev=previousTrainingLog(ex.id,date);let cue='Registrá una referencia limpia hoy; después el entrenador va a comparar sesiones.';
